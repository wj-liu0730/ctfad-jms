...
def test_loop(cfg, algo, tf_test_dataset, writer):
    ...
    top_n = 50  # take the top 100 boxes as the final result
    ...
    for test_data in tf_test_dataset:
        ...
        invalid_num_pre = np.count_nonzero(predictions['classes'] == -1)
        valid_pre = top_n - invalid_num_pre
        predictions['number_detections'] = (valid_pre,)
        # Overwrite to one label
        predictions['classes'][:, :valid_pre] = np.zeros_like(predictions['classes'][:, :valid_pre])

        invalide_num_gt = np.count_nonzero(np_labels['classes'] == -1)
        valid_gt = top_n - invalide_num_gt
        # Overwrite to one label
        np_labels['classes'][:, :valid_gt] = np.zeros_like(np_labels['classes'][:, :valid_gt])

        algo.evaluattion_metrics.update_state(np_labels, predictions)
